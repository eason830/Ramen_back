...
```
## views新增四個檔案，Admin.vue，AdminHome.vue，AdminProducts.vue，
...
export default {
  ...
  methods: {
    async submitModal (event) {
      ...
      try {
        if (this.form._id.length === 0) {
          ...
          this.$refs.table.refresh() //告訴BV，table需要更新，上面table標籤要加ref
        }

        this.$bvModal.hide('modal-product') //關掉modal
      } catch (error) {
        this.$swal({
          icon: 'error',
          title: '錯誤',
    ...
    editProduct (index) {
      ...
      this.$bvModal.show('modal-product') // 把modal打開來
    }
  },
  // 顯示商品
  async created () {
